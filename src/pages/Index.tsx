...
function ProductCard({ listing, wishlist, onWishlist }: {
  listing: any; wishlist: Set<string>; onWishlist: (id: string) => void;
}) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-muted/40">
        ...
        {listing.is_featured && (
          <Badge className="absolute left-3 top-3 bg-primary text-primary-foreground text-[10px] px-2 py-0.5 rounded-full">Featured</Badge>
        )}
    ...
    <Link to={`/listing/${listing.id}`} className="group block">
      <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-muted/60 to-muted/20 border shadow-2xl">
        <div className="aspect-[3/4] w-full overflow-hidden">
          {listing.images?.[0]
            ? <img src={listing.images[0]} alt={listing.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
...
const Index = () => {
  ...
  const { data: heroListing } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("listings").select("*").eq("is_featured", true).eq("is_active", true).order("created_at", { ascending: false }).limit(1).single();
      return data;
    },
  });
      ...
      <div className="min-h-screen pb-20 md:pb-0">
            ...
            <div className="grid gap-8 md:grid-cols-2 md:items-center xl:gap-16">
              <div>
                ...
                <motion.h1 {...fadeUp(0.1)} className="mt-5 font-display text-[2.6rem] font-black leading-[1.05] tracking-tight text-foreground md:text-6xl xl:text-7xl">
                  Shop Smart.<br /><span className="text-primary">Buy Local.</span><br />
                ...
                <motion.div {...fadeUp(0.4)} className="mt-8 flex gap-8 xl:gap-12">
                  {[{ value: "500+", label: "Students" }, { value: "1.2k+", label: "Listings" }, { value: "4.9★", label: "Rating" }].map((s) => (
                    <div key={s.label}>
              ...
              <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} className="flex justify-center md:justify-end">
                {heroListing ? (
                  <div className="w-full max-w-xs xl:max-w-sm"><HeroProductCard listing={heroListing} /></div>
                ) : (
                    ...
                    <div className="relative overflow-hidden rounded-[2rem] border bg-gradient-to-br from-primary/10 via-card to-secondary/10 shadow-2xl">
                      <div className="flex aspect-[3/4] flex-col items-center justify-center gap-4 p-8">
                        ...
                        <div className="text-center">
                          <p className="font-bold text-foreground text-lg">CampusMart</p>
                        ...
                        <Link to="/marketplace" className="w-full">
                          <Button className="w-full rounded-2xl gap-2 font-bold">Browse All <ArrowRight className="h-4 w-4" /></Button>
                        </Link>
                      </div>
                    </div>
                  </div>
        ...
        <section className="px-5 py-6 md:px-12 xl:px-20">
          <div className="mx-auto max-w-[1400px] grid gap-4 sm:grid-cols-2">
            <motion.div {...stagger(0)} className="group relative flex items-center justify-between overflow-hidden rounded-3xl bg-gradient-to-r from-primary to-primary/70 p-6 shadow-lg cursor-pointer" onClick={() => navigate("/marketplace")}>
              ...
              <div>
                ...
                <span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-primary-foreground underline underline-offset-2">Shop Now <ArrowRight className="h-3 w-3" /></span>
              </div>
              <span className="text-6xl select-none">🚚</span>
            </motion.div>
            <motion.div {...stagger(1)} className="group relative flex items-center justify-between overflow-hidden rounded-3xl bg-gradient-to-r from-zinc-900 to-zinc-700 dark:from-zinc-800 dark:to-zinc-600 p-6 shadow-lg cursor-pointer" onClick={() => navigate("/register")}>
              ...
              <div>
                ...
                <span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-white underline underline-offset-2">Get Deal <ArrowRight className="h-3 w-3" /></span>
              </div>
              <span className="text-6xl select-none">🎓</span>
            </motion.div>
          </div>
        ...
        <section className="px-5 py-6 md:px-12 xl:px-20">
          <div className="mx-auto max-w-[1400px]">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/marketplace" className="text-xs text-primary font-semibold flex items-center gap-1 hover:underline">All <ArrowRight className="h-3 w-3" /></Link>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-5 px-5 md:mx-0 md:px-0">
              <button onClick={() => setActiveCategory("all")} className={`flex h-10 shrink-0 items-center gap-2 rounded-full px-5 text-sm font-semibold transition-all ${activeCategory === "all" ? "bg-primary text-primary-foreground shadow-md shadow-primary/30" : "bg-card border text-foreground hover:border-primary/40"}`}>All</button>
              {CATEGORIES.map((cat) => (
                <button key={cat.value} onClick={() => setActiveCategory(cat.value)} className={`flex h-10 shrink-0 items-center gap-2 rounded-full px-5 text-sm font-semibold transition-all ${activeCategory === cat.value ? "bg-primary text-primary-foreground shadow-md shadow-primary/30" : "bg-card border text-foreground hover:border-primary/40"}`}>
        ...
        <section className="px-5 pb-10 md:px-12 xl:px-20">
          <div className="mx-auto max-w-[1400px]">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="font-display text-2xl font-bold xl:text-3xl">{activeCategory === "all" ? "All Products" : CATEGORIES.find(c => c.value === activeCategory)?.label}</h2>
                <p className="text-xs text-muted-foreground mt-0.5">{listings?.length ? `${listings.length} items` : ""}</p>
              </div>
              <Link to={`/marketplace${activeCategory !== "all" ? `?category=${activeCategory}` : ""}`}>
                <Button variant="ghost" size="sm" className="gap-1 text-primary text-xs">See all <ArrowRight className="h-3.5 w-3.5" /></Button>
              </Link>
            </div>
            {isLoading ? (
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
                {Array.from({ length: 8 }).map((_, i) => <div key={i} className="rounded-3xl bg-muted/40 animate-pulse aspect-[3/4]" />)}
              </div>
            ) : listings && listings.length > 0 ? (
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
                {listings.map((listing: any) => <ProductCard key={listing.id} listing={listing} wishlist={wishlist} onWishlist={toggleWishlist} />)}
              </div>
            ) : (
              <div className="rounded-3xl border bg-muted/20 py-20 text-center">
                ...
                <Link to="/create-listing"><Button className="mt-5 rounded-2xl gap-2"><Package className="h-4 w-4" /> List an Item</Button></Link>
              </div>
            )}
          </div>
        </section>
                  ...
                  <Link key={listing.id} to={`/listing/${listing.id}`} className="shrink-0 w-40 group">
                    <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                      <div className="aspect-square overflow-hidden bg-muted/40">
                        {listing.images?.[0] ? <img src={listing.images[0]} alt={listing.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" /> : <div className="flex h-full items-center justify-center text-3xl">📦</div>}
                      </div>
                      <div className="p-2.5">
        ...
        <section className="bg-muted/30 px-5 py-14 md:px-12">
          <div className="mx-auto max-w-6xl">
            <div className="text-center mb-10">
              <h2 className="font-display text-2xl font-bold md:text-3xl">Why CampusMart?</h2>
            ...
            <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-4">
              {[
                ...
                <motion.div key={item.title} {...stagger(i)} className="rounded-3xl border bg-card p-6 shadow-sm">
                  <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${item.color}`}><item.icon className="h-5 w-5" /></div>
            ...
            <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 p-8 shadow-2xl md:p-12">
              ...
              <div className="relative flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <h2 className="mt-3 font-display text-3xl font-black text-white md:text-4xl">Support Student<br />Businesses</h2>
                  <p className="mt-2 max-w-sm text-sm text-white/60">Hundreds of student vendors selling on campus. List your items for free and start earning today.</p>
                  <div className="mt-6 flex flex-wrap gap-3">
                    <Link to="/create-listing"><Button size="lg" className="rounded-2xl gap-2 font-bold"><Package className="h-4 w-4" /> Start Selling</Button></Link>
                    {!user && <Link to="/register"><Button size="lg" variant="outline" className="rounded-2xl border-white/20 text-white hover:bg-white/10">Register Free</Button></Link>}
                  </div>
                </div>
